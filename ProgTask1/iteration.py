...
#Function that calculate the P matrix for iteration
def p_calculation(matrix: np.arry) :
    matrix_transpose = matrix.transpose
    p_matrix = np.dot(np.dot(matrix_transpose ,np.linalg.inv(np.dot(matrix, matrix_transpose))), matrix)
    return p_matrix
